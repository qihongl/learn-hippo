...
source_file="$repo_root/docs/learned_encoding/experiment_report.md"
style_file="$repo_root/docs/learned_encoding/paper.css"
output_file="$repo_root/docs/learned_encoding/experiment_report.html"
title_filter="$repo_root/scripts/pandoc_report_title.lua"

(
  cd "$repo_root/docs/learned_encoding"
  pandoc "$(basename "$source_file")" \
    ...
    --css="$style_file" \
    --resource-path="$repo_root/docs/learned_encoding:$repo_root" \
    --output="$output_file"
)
...
assert document.count("<math") >= 4, "expected MathML equations"
