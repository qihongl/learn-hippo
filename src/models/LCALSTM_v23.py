...
class LCALSTM(nn.Module):
    ...
    def __init__(
        ...
        # memory
        self.hpc = nn.Linear(
            rnn_hidden_dim + dec_hidden_dim + 1, dec_hidden_dim)
        self.hpc_h = nn.Linear(dec_hidden_dim, N_SSIG)
    ...
    def forward(self, x_t, hc_prev, beta=1):
        ...
        inps_t = sigmoid(self.hpc_h(sigmoid(self.hpc(hpc_input_t))))
    ...
    def recall(self, c_t, inps_t):
        ...
        if self.em.retrieval_off:
            ...
            m_t = self.em.get_memory(
                c_t, leak=0, comp=self.comp_t, w_input=inps_t
            )
        return m_t
